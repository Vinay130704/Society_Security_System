...
const ResidentDashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const [eventsRes, visitorsRes, deliveriesRes, staffRes] = await Promise.all([
        axios.get(`${API_BASE_URL}/event/view-event?past=false`, getAuthHeaders()),
        axios.get(`${API_BASE_URL}/visitor/pending-approvals`, getAuthHeaders()),
        axios.get(`${API_BASE_URL}/delivery/all`, getAuthHeaders()),
        axios.get(`${API_BASE_URL}/staff/resident/${residentData?._id || 'temp'}`, getAuthHeaders())
      ]);
  ...
  const fetchRecentAlerts = async () => {
    try {
      const promise = axios.get(`${API_BASE_URL}/emergency/my-alerts`, getAuthHeaders());
  ...
  const fetchUpcomingEvents = async () => {
    try {
      const promise = axios.get(`${API_BASE_URL}/event/view-event?past=false&limit=2`, getAuthHeaders());
      const response = await toast.promise(
        promise,
        {
  ...
  const fetchLogData = async () => {
    try {
      const [residentLogsRes, staffLogsRes] = await Promise.all([
        axios.get(`${API_BASE_URL}/profile/logs/${residentData?.permanentId || 'temp'}`, getAuthHeaders()),
        axios.get(`${API_BASE_URL}/staff/history/${residentData?.permanentId || 'temp'}`, getAuthHeaders())
      ]);
  ...
  const triggerEmergency = async () => {
    try {
      const promise = axios.post(
        `${API_BASE_URL}/emergency/create-emergency`,
        {
          type: 'Panic',

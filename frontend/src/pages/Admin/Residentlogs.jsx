...
const ResidentLogsView = () => {
    ...
    const fetchSuggestions = async (query) => {
        try {
            ...
            const response = await axios.get(
                "http://localhost:5000/api/profile/pid-suggestions",
                {
                    params: { query },
    ...
    const fetchLogs = async () => {
        try {
            ...
            let endpoint = "/api/profile/logs/all";
            if (filters.permanentId) {
                endpoint = `/api/profile/logs/${filters.permanentId}`;
            }
            ...
            if (response.data?.success) {
                ...
                setPagination({
                    ...
                    page: filters.page,
                    limit: filters.limit,
                    totalPages: Math.ceil(response.data.logs.length / filters.limit),
                });
            } else {
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex items-center">
                                                        {log.user?.profilePicture ? (
                                                            <img
                                                                src={`http://localhost:5000/${log.user.profilePicture}`}
                                                                alt={log.personName}
